    ...
    $completions = @(switch ($command)
        {
            ...
            'wezterm;cli'
            {
                ...
                [CompletionResult]::new('split-pane', 'split-pane', [CompletionResultType]::ParameterValue, 'split the current pane. Outputs the pane-id for the newly created pane on success')
                [CompletionResult]::new('spawn', 'spawn', [CompletionResultType]::ParameterValue, 'Spawn a command into a new window or tab Outputs the pane-id for the newly created pane on success')
            ...
            'wezterm;imgcat'
            {
                ...
                [CompletionResult]::new('--height', 'height', [CompletionResultType]::ParameterName, 'Specify the display height; defaults to "auto" which automatically selects an appropriate size.  You may also use an integer value `N` to specify the number of cells, or `Npx` to specify the number of pixels, or `N%` to size relative to the
terminal height')
...
Set-PSReadLineKeyHandler -Key Ctrl+w `
    -BriefDescription "List Worktrees" `
    -LongDescription "Delete the last word in the current line, or the last character if the cursor is at the end of the line" `
    -ScriptBlock { 
    git worktree list | awk '{print $1}' | fzf | Set-Location
}
